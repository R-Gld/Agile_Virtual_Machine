...
Feature: JajaCode Interpretation
  ...
  @debug @memory
  Scenario: Verify complex program state with method calls and loops
    Given a JajaCode program:
      """
      1 init
      2 push(w)
      ...
      6 push(5)
      7 new(VAL@global, int, cst, 0)
      8 push(true)
      9 new(b1@global, boolean, var, 0)
      10 push(false)
      11 new(b2@global, boolean, var, 0)
      12 push(15)
      13 new(somme, int, meth, 0)
      14 goto(35)
      15 new(max@somme@int, int, var, 1)
      16 load(VAL@global)
      17 new(t@somme@int, int, var, 0)
      18 load(max@somme@int)
      19 push(0)
      20 sup
      21 not
      22 if(30)
      23 load(max@somme@int)
      24 inc(t@somme@int)
      25 load(max@somme@int)
      26 push(1)
      27 sub
      28 store(max@somme@int)
      29 goto(18)
      30 load(t@somme@int)
      31 swap
      32 pop
      ...
      34 return
      35 push(38)
      36 new(test, boolean, meth, 0)
      37 goto(45)
      38 new(b@test@boolean, int, var, 1)
      39 new(a@test@boolean, int, var, 2)
      40 load(a@test@boolean)
      41 load(b@test@boolean)
      42 sup
      43 swap
      44 return
      45 push(48)
      46 new(f, void, meth, 0)
      47 goto(62)
      48 new(z@f@void, int, var, 1)
      49 load(y@global)
      50 load(z@f@void)
      51 mul
      52 store(x@global)
      ...
      """
    When I initialize the debugger
    And I execute steps until address 88
    And I capture the debug memory snapshot
    Then the snapshot should show variable "f" with value 48
    And the snapshot should show variable "test" with value 38
    And the snapshot should show variable "somme" with value 15
    And the snapshot should show variable "b2@global" with value false
    And the snapshot should show variable "b1@global" with value true
    And the snapshot should show variable "VAL@global" with value 5
    And the snapshot should show variable "y@global" with value 10
    And the snapshot should show variable "x@global" with value 5
